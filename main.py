...
""")
			...
			if len(parts) == 3 and parts[1] == "-i":
				raw = parts[2]
				"""if raw.lower() == "all":
					answer = input(f"[*] This will delete all active sessions, are you sure y/N > ")

					if answer.lower() in ("yes", "y"):
						sid = raw
						os_type = "killall"
						if session_manager.kill_http_session(sid, os_type):
							print(brightyellow + f"[*] Killed all active sessions!")

						else:
							print(brightred + f"[!] Failed to kill all active sessions!")"""
